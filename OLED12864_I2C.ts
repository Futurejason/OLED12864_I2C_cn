...
namespace OLED12864_I2C {
    ...
    //  * @param chr is a str, eg: 'm'
    //  */
    // //% blockId="OLED12864_I2C_PIXEL" block="显示字符 x坐标 %x|y坐标 %y|字符内容 %chr|字体大小 %size1 "
    // //% weight=70 blockGap=8
    // //% parts=OLED12864_I2C trackArgs=0
    // export function OLED_ShowChar(x: number, y: number, str: String, size1:number) {
    //     let i, m, temp, chr1: number, size2;
    //     let y0 = y;
    //     let chr = str.charCodeAt(0);
    //     let kg = ' ';
    //     size2 = (size1 / 8 + ((size1 % 8) ? 1 : 0)) * (size1 / 2);  //得到字体一个字符对应点阵集所占的字节数
    //     chr1 = chr - kg.charCodeAt(0);  //计算偏移后的值
    //     for (i = 0; i < size2; i++) {
    ...
    //% parts=OLED12864_I2C trackArgs=0
    //% shim=Oledcppbook::OLED_Init
    export function OLED_Init(scls: DigitalPin, sdas: DigitalPin) {
        return;
